...
kony.apps.coe.ess.Sync.deltaSync=function(){
  ...
  if (kony.application.getCurrentForm().id != "frmLogin") {
    // Force axway and backend login refresh, followed by sync initialization, before attempting to sync data
    kony.apps.coe.ess.frmLogin.axwayAuth(null, "", function(){
      //kony.application.dismissLoadingScreen();
    ...
    function(err) {
        //kony.application.dismissLoadingScreen();
        kony.sdk.mvvm.log.error("Error when refreshing axway/backend login refresh");
        handleError(err);
    }, true);
  }
