		...
		fetchData: function () {
			try {
				...
				var comments_qurey = "SELECT [request_note].[id] AS [ID], " +
                    ...
                    "CASE WHEN (Employee.First_Name = '' OR  Employee.First_Name ISNULL) THEN "+
                    "(select value from attribute where approval_id='" + selectedApprovalID + "' and attribute_def_id like 'FirstNameAttributeDef') "+
                    "ELSE Employee.First_Name END AS FirstName, "+
                	"CASE WHEN (Employee.Last_Name = '' OR  Employee.Last_Name ISNULL) "+
                    "THEN (select value from attribute where approval_id='" + selectedApprovalID + "' and attribute_def_id like 'LastNameAttributeDef') "+
                    "ELSE Employee.Last_Name END AS LastName, "+
				...
				if (ApprovalRequestDetailData && ApprovalRequestDetailData.RequestDetials && ApprovalRequestDetailData.RequestDetials.request_type && ApprovalRequestDetailData.userAttachments) {
					...
					if (ApprovalRequestDetailData.userAttachments && ApprovalRequestDetailData.userAttachments.length && ApprovalRequestDetailData.userAttachments.length > 0) {
						var WidgetDatamap = {
                          ...
                          var attachmentNum=parseInt(imageindex)+1;
                   		ApprovalRequestDetailData.userAttachments [imageindex].lblAttachmentName ={
                          		"text" : kony.i18n.getLocalizedString("i18n.ess.myApprovals.PdfReader.Attachment")+attachmentNum,
                            };
                        }
