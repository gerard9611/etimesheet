 ...
 */
  ...
  * security attributes
  */
  kony.apps.coe.ess.frmLogin.axwayAuth = function(data, action, successCallBack, errorCallback, isRefresh) {
   try {
 ...
 kony.apps.coe.ess.frmLogin.oktaRefresh = function(refreshToken, action, successCallBack, errorCallback, isRefresh) {
  try {
      ...
      refreshLoginService.login({"refresh_token":refreshToken}, function(data){
        ...
        kony.apps.coe.ess.frmLogin.axwayAuth(data, action, successCallBack, errorCallback, isRefresh);
      } , function(err){
        if(isRefresh === null || isRefresh === undefined || isRefresh ===false){
          kony.application.dismissLoadingScreen();
        }
...
kony.apps.coe.ess.frmLogin.oktaLogin = function() {
  try {
    ...
    if (kony.apps.coe.ess.globalVariables.active_login_service !== undefined && kony.apps.coe.ess.globalVariables.active_login_service !== null && kony.apps.coe.ess.globalVariables.active_login_service !== "" && kony.apps.coe.ess.frmLogin.username !== undefined && kony.apps.coe.ess.frmLogin.username !== null && kony.apps.coe.ess.frmLogin.username !== "") {
      frmLogin.lblApploadTime.setVisibility(false);
	    frmLogin.lblwait.setVisibility(false);
      ...
      function startOktaLogin() {
        ...
        if (isRememberMeChecked && kony.store.getItem("oktaToken") !== null && kony.store.getItem("oktaToken") !== undefined) {
          ...
          }, kony.apps.coe.ess.frmLogin.axwayAuth, kony.apps.coe.ess.frmLogin._errorCallback);
        }
      };
      if(kony.sdk.isInitialized === true) {
        startOktaLogin();
      } else {
        initMbaasApp(startOktaLogin);
      }
    }
    }else{
				popupNetworkAlert.show();
...
kony.apps.coe.ess.frmLogin.loginRefresh = function() {
  function loginRefreshSuccessCallback() {
    ...
    // Initialize sync after the login refresh
    kony.apps.coe.ess.Sync.initializeSync(kony.apps.coe.ess.Sync.deltaSyncConfig,
    function(err) {
  ...
  function startLoginRefresh() {
    //Stop any sync operations from occurring
    try {
  ...
  if(kony.sdk.isInitialized === true) {
    ...
    initMbaasApp(startLoginRefresh);
  }
};
